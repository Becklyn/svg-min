...
const ALLOWED_ROOT_ATTRIBUTES = /^(xmlns|viewBox|class|width|height)$/;
...
module.exports = class Optimizer
{
    ...
    inline ()
    {
        ...
        const offset = this.moveViewBoxToOriginAndGetOffset(this.svgDocument, rbox);

        this.svgDocument.children().forEach(
            child => this.moveToOrigin(child, -offset.x, -offset.y)
        );
    ...
    /**
     * Moves the viewbox to the origin and returns the total offset for children
     *
     * @private
     * @param {svgjs.Doc} root
     * @param {RBox} rbox
     * @returns {{x: number, y: number}}
     */
    moveViewBoxToOriginAndGetOffset (root, rbox)
    {
        ...
        for (let i = 0; i < children.length; i++)
        {
            ...
            if (child.node.nodeName.toLowerCase() === "svg")
            {
                const viewBox = child.viewbox();
                child.viewbox(0, 0, viewBox.width, viewBox.height);
